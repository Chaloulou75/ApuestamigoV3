...
<div class="w-full md:w-1/2 mx-auto">
    ...
    <form class="animate__animated animate__fadeInUp bg-francagris px-6 py-6 mb-1" method="POST"
        ...
        <div class="border-t-4 border-francaverde rounded bg-francagris py-4 mb-4 flex flex-col justify-between">
            ...
            <input id="name" name="name" type="text"
                class="bg-teal-100 rounded text-left text-gray-900 text-sm font-medium px-2 py-2 @error('name') bg-red-dark @enderror"
        ...
        <div class="border-t-4 border-francaverde rounded bg-francagris py-4 mb-4 flex flex-col justify-between">
            ...
            <input id="email" name="email" type="email"
                class="bg-teal-100 rounded text-left text-gray-900 text-sm font-medium px-2 py-2"
        ...
        <div class="border-t-4 border-francaverde rounded bg-francagris py-4 mb-4 flex flex-col justify-between">
            ...
            <label for="equipe_id" class="text-left text-white text-sm font-medium"> {{ __('all.Favorite Club') }} :
            </label>

            <select id="equipe_id"
                class="bg-teal-100 rounded text-left text-gray-900 text-sm font-medium px-2 py-2 {{ $errors->has('equipe_id') ? ' bg-red-dark' : '' }}"
                name="equipe_id" value="">

                @foreach($equipes as $equipe)

                <option value="{{ $equipe->id}}" @if($user->equipe_id == $equipe->id)selected @endif>{{ $equipe->name}}
                </option>
                @endforeach
            </select>
    ...
    <div class="px-6 py-4 mb-4 mx-6 flex flex-col justify-between ">
        <div class="divide-y divide-francaverde">
            ...
            @if($loop->first)
            <h3 class="text-left text-white text-base font-medium uppercase py-2 px-4">{{$loop->count}}
                {{ __('all.leagues') }}:</h3>
            @endif
            <div
                class="text-white hover:text-francaverde font-medium tracking-wide text-sm truncate py-2 transition duration-500 ease-in-out transform hover:translate-x-1 px-4">
                <a href="{{ route('ligues.show', $ligue) }}">
                    {{ $ligue->name }} <span
                        class="text-xs font-thin italic text-gray-300">({{ $ligue->championnat->name}})</span>
    ...
    <form class="bg-francagris px-6 py-4 mb-4" method="POST" action="{{ route('profile.destroy', $user) }}">
        ...
        <div class="border-2 border-francaverde rounded bg-francagris p-4 flex flex-col justify-between">
            <p class="block text-white text-sm font-medium mb-2">
                {{ __('all.Please note, this action is irreversible.') }} </p>
            <button
                class="w-full bg-julienred text-white hover:text-francaverde font-medium py-2 px-4 border-2 border-francaverde rounded focus:outline-none focus:shadow-outline"
                type="submit">
                <svg class="h-6 w-6 inline-block pr-1" fill="none" stroke-linecap="round" stroke-linejoin="round"
                    stroke-width="2" viewBox="0 0 24 24" stroke="currentColor">
                    <path
