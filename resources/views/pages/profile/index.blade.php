...
<div class="w-full md:w-4/5 mx-auto my-4">
    ...
    <div class="text-white text-sm">
        <ul class="list-disc">
            ...
            <li class="animate__animated animate__lightSpeedInLeft py-4 px-2">id <span
                    class="text-blue-500">{{$user->id}}</span>: <span class="text-francaverde">{{$user->name}}</span> -
                <span class="text-gray-500">({{$user->email }} / admin: {{ $user->admin }} / créé le:
                    {{ \Carbon\Carbon::parse($user->created_at)->isoFormat('dddd Do MMMM YYYY H:mm') }})</span> dont
                l'équipe préférée est <span class="text-gray-500">{{$user->equipe->name}} </span> <img
                    class="inline w-10 h-10" loading="lazy"
                    src="{{ $user->equipe->logourl ? url($user->equipe->logourl) : URL::to('/img/' .$user->equipe->logo) }}"></span>
