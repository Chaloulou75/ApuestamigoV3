...
<div class="w-full md:w-4/5 mx-auto">
    ...
    <div class="border-t-4 border-double border-francaverde bg-francagris">
        <h1
            class="animate__animated animate__flipInX text-center text-white text-3xl tracking-wider font-semibold my-4">
            {{ $championnat->name }}
        </h1>
    </div>
    ...
    <p class="text-base text-center text-francaverde py-2">
        <a href="{{route('championnats.index')}}">
            Retourner à la liste des championnats
        </a>
    </p>
    ...
    <div class="w-full lg:w-1/2 mx-auto bg-francagris shadow-md border-2 border-white rounded py-8 px-8 my-4">
        <form class="mx-auto" method="POST" enctype="multipart/form-data"
            action="{{ action('ChampionnatController@update', $championnat) }}">
            @csrf
            @honeypot
            @method('PUT')
            ...
            <div class="mb-4">
                <label for="name"
                    class="block text-white text-sm font-medium tracking-wide mb-2">{{ __('Nom du championnat') }}</label>
                ...
                <input id="name" type="text" class="shadow appearance-none border rounded w-full py-2 px-3 text-julien-gris leading-loose focus:outline-none focus:shadow-outline
	             @error('name') bg-red-dark @enderror" name="name" value="{{ $championnat->name }}" required
                    autocomplete="name">
            ...
            <div class="mb-4">
                <label for="logo"
                    class="block text-white text-sm font-medium tracking-wide mb-2 ">{{ __(' logo du championnat') }}</label>
                <div class="flex">
                    <input id="logo" type="file"
                        class="shadow appearance-none border rounded w-full py-2 px-3 text-white leading-loose focus:outline-none focus:shadow-outline @error('logo') bg-red-dark @enderror"
                        name="logo" value="" autocomplete="logo">

                    <img src="{{ $championnat->logourl ? url($championnat->logourl) : URL::to('/img/' .$championnat->logo) }}"
                        loading="auto" class="h-12 w-12 object-scale-down" alt="logo">
                </div>
