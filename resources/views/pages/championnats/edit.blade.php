...
<div class="w-full md:w-4/5 mx-auto">
		...
		<div class="border-t-4 border-double border-francaverde bg-francagris">
	  		<h1 class="animate__animated animate__flipInX text-center text-white text-3xl tracking-wider font-semibold my-4">{{ $championnat->name }}</h1>
	  	</div>
		...
		<div class="w-full lg:w-1/2 mx-auto bg-francagris shadow-md border-2 border-white rounded py-8 px-8 my-4">
		  <form class="mx-auto" method="POST" enctype="multipart/form-data" action="{{ action('ChampionnatController@update', $championnat) }}">
		  		@csrf
		  		@honeypot
		  		@method('PUT')
		      ...
		      <label for="name" class="block text-white text-sm font-medium tracking-wide mb-2">{{ __('Nom du championnat') }}</label>
	            ...
	            <input id="name" type="text" class="shadow appearance-none border rounded w-full py-2 px-3 text-julien-gris leading-loose focus:outline-none focus:shadow-outline
	             @error('name') bg-red-dark @enderror" name="name" value="{{ $championnat->name }}" required autocomplete="name">
		      ...
		      <div class="mb-4">
				...
				<div class="flex">
		            ...
		            <img src="{{ $championnat->logourl ? url($championnat->logourl) : URL::to('/img/' .$championnat->logo) }}" loading="auto" class="h-12 w-12 object-scale-down" alt="logo">
		        </div>
