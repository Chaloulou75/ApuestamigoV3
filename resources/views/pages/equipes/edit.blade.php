...
<div class="w-full md:w-4/5 mx-auto">
    ...
    <div class="w-full lg:w-1/2 mx-auto bg-francagris shadow-md border-2 border-white rounded py-8 px-8 my-4">
        <form class="mx-auto" method="POST" enctype="multipart/form-data"
            action="{{ action('EquipeController@update', $equipe) }}">
            @csrf
            @honeypot
            @method('PUT')
            <div class="mb-4">
                ...
                <select id="championnat_id"
                    ...
                    name="championnat_id" value="{{ old('championnat_id') }}" placeholder="{{$equipe->championnat_id}}"
                    required>
            ...
            <div class="mb-4">
                <label for="name"
                    class="block text-white text-sm font-medium tracking-wide mb-2">{{ __('Nom d\'équipe') }}</label>
                ...
                <input id="name" type="text" class="shadow appearance-none border rounded w-full py-2 px-3 text-julien-gris leading-loose focus:outline-none focus:shadow-outline
	             @error('name') bg-red-dark @enderror" name="name" value="{{ $equipe->name }}" required
                    autocomplete="name">
            ...
            <div class="mb-4">
                <label for="logo"
                    class="block text-white text-sm font-medium tracking-wide mb-2 ">{{ __(' logo du club') }}</label>
                <div class="flex">
                    ...
                    <img src="{{ $equipe->logourl ? url($equipe->logourl) : URL::to('/img/' .$equipe->logo) }}"
