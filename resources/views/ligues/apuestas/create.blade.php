...
<div class="w-full text-francagris md:px-4">
    ...
    <h3 class="text-base text-left text-white tracking-wide px-2 py-2">{{__('all.Hey')}} <span
            class="text-francaverde">{{$user->name}}</span>, {{__('all.let\'s play')}}.</h3>
    @endauth
    @guest
    <h3 class="text-base text-left text-francaverde tracking-wide px-2 py-2">{{__('all.Hey')}},
        {{__('all.you have to login or register to play.')}}</h3>
    @endguest
    ...
    <form method="POST" action="{{ action('ApuestasController@store', $ligue) }}">
        ...
        <div class="text-center">
            <button type="submit"
                class="w-3/4 bg-francagris text-white hover:text-francaverde font-medium text-center tracking-widest border-2 rounded-lg border-francaverde py-2 px-6 my-4">
                {{__('all.Registrar')}}
            </button>
        </div>
    </form>
