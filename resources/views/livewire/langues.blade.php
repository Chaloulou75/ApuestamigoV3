<div x-data="{ open: false }" class="relative">
    <button x-on:click="open = true" class ="mt-1 block px-2 py-1 text-sm focus:outline-none hover:text-francaverde md:mt-0 md:ml-2 transition duration-500 ease-in-out transform hover:translate-x-2"
            >
  ...
  >
      <div class="absolute z-50 mt-2 p-2 w-40 bg-francagris border-2 border-francaverde rounded-lg text-sm shadow-xl">
        ...
        @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
            ...
            >
		        <img class="inline-block h-5 w-5 border border-francaverde focus:outline-none focus:border-white rounded-full object-cover" loading="lazy" alt="{{ $properties['native'] }}" src="{!! asset('img/flags/' . $localeCode. '.png') !!}"/>
