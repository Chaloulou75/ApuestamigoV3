	...
	<table class="table-auto w-full bg-white shadow-md border-t-4 border-solid rounded-lg border-francaverde mb-4">
	  ...
	  <tbody>
	    ...
	    <tr class="@if( $user->id == Auth::user()->id ) bg-blue-200 @endif">
		  ...
		  <td class="py-2 px-4">				  	
		  	<img class="inline w-10 h-8 pr-2" loading="lazy" src="{{ $user->equipe->logourl ? url($user->equipe->logourl) : URL::to('/img/' .$user->equipe->logo) }}" loading="lazy" alt="{{ $user->equipe->name}}"> 
