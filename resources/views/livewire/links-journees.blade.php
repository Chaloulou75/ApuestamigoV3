<div class="m-2 relative" x-data="{ show: false }" x-on:click.away="show = false">
    ...
    <div class="absolute z-30 bg-francagris text-white py-2 rounded mt-1"
    	...
    	<a class="transition duration-500 ease-in-out transform hover:translate-x-1 block hover:text-francaverde text-xs py-2 px-8" href="{{ action('ApuestasController@show', [$ligue, $user, $fecha]) }}">
            {{ $fecha->namejournee }}
        </a>
