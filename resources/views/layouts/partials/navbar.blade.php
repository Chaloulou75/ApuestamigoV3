  ...
  <nav class="md:block" x-bind:class="{ 'block': isOpen, 'hidden': !isOpen }"
    ...
    {{-- dropdown --}}
      ...
      <div x-data="{ open: false }" class="relative hidden md:block md:ml-6">
            <button x-on:click="open = true" class="z-10 block h-8 w-8 rounded-full overflow-hidden border-2 border-francaverde focus:outline-none focus:border-francaverde">
          ...
          >
              <div class="absolute z-50 right-0 mt-2 py-2 w-48 bg-francagris border-2 border-francaverde rounded-lg text-sm shadow-xl">
                ...
                <a href="{{ route('profile.show', auth()->user()->id) }}" class="px-4 py-2 text-white hover:text-francaverde transition duration-500 ease-in-out transform hover:translate-x-2 flex items-center">
    ...
    <div class="px-2 py-1 md:hidden">
      @auth
      <div class="flex items-center mt-1 px-2 py-2 text-sm leading-none rounded transition duration-500 ease-in-out transform hover:translate-x-2">
